...
# Önceki adımdan results'ı yükle
with open("results.pkl", "rb") as f:
    results = pickle.load(f)
print(f"Yüklendi: {len(results)} dosya")

# F0 hesaplama fonksiyonu (otokorelasyon)
...
print("\nF0 İstatistikleri:")
for sinif, f0_list in f0_by_class.items():
    print(f"  {sinif:8s} → Ort: {np.mean(f0_list):.1f} Hz  |  Std: {np.std(f0_list):.1f} Hz  |  N: {len(f0_list)}")

# Kaydet
with open("results_f0.pkl", "wb") as f:
    pickle.dump(results, f)
print("\nresults_f0.pkl kaydedildi!")